		...
		return this.each(function(id,el){
			...
			$.fn.extend({
				...
				removeMenuItem: function(){
					return this.each(function(){
						...
						tvmenu.confirm(conf.phrase.warning,conf.phrase.removeItemWarning,{confirm: removeItem});
					});
				},
				...
				eventOnClickAddLink: function(){
					return this.each(function(){
						...
						$this.updateNoChildrenClass();
						$('.content-row',$cloneItem).tvAccordion();
					});
				},
			...
			function menuItemTypeTmpl(){
				...
				$('[data-type="category"]',$menuTypeList).each(function(){
					var $field = $(this);
					tvInstantiateChooser($field.attr('id'));
				});
				...
				$(conf.items,$menuTypeList).draggable({
					...
					stop: function(event, ui) {
						...
						$('.content-row',ui.helper).tvAccordion();
					}
				});
			};
			...
			function menuActions(){
				...
				$previewBtn.on('click',function(){
					if(previewUrl != null){
						var menuJson = tvmenu.menuItemToJson();
							...
							opened: function () {
								...
								$.ajax({
									...
									success: function(res){
										...
										('.tv-menu',$previewArea).first().trigger('contentUpdated')
			...
			function attachIconToItemHeading(textEl,value, type){
				...
				if(type == 0){
					...
					if($textEl.data('name') == 'icon_img'){
						$previewIcon.html('<a class="preview-image" onclick="tvmenu.viewfull(this)" href="javascript:void(0)" data-href="'+value+'"><img src="'+value+'" /></a>');
					}
			...
			$.fn.tvAccordion = function(options){
				/*var defaultConfig = {
				...
				return this.each(function(){
					...
					$(apiConfig.header,$this).each(function(){
						var $title = $(this);
						if(typeof $title.data('tvAccordion') == 'undefined'){
							...
							$title.data('tvAccordion',true);	
						}
					});					
				});*/
			}
			$('.content-row').tvAccordion();
			...
			eval('window.'+conf.windowId+' = tvmenu');
		});
	};
})(jQuery);