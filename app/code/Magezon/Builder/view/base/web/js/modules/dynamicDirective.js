...
(function(dynamicDirective) {
  ...
  function DynamicDirectiveContext($compile, dynamicDirectiveManager, controller, elementName, attrs) {
    ...
    this.recompileInnerElement = function(scope, element, suffix) {
      ...
      if (scope.$parent.element && scope.element) {
        var parallaxHtml = '<div class="mgz-parallax ' + scope.element.id + '-p"><div class="mgz-parallax-inner"></div></div>';
        element.append(parallaxHtml);
      }
  ...
  function DynamicDirectiveManagerProvider() {
    ...
    this.$get = function($injector) {
      return $injector.instantiate(DynamicDirectiveManager, {
        $compileProvider: $compileProvider
      });
    };
  }
