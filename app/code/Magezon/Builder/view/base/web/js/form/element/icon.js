define([
	...
	return {
		...
		controller: ['$rootScope', '$scope', 'magezonBuilderService', function($rootScope, $scope, magezonBuilderService) {
		...
		link: function(scope, element, attrs, $select, $timeout) {
            element.find('.mgz-selector-button').click(function(event) {
            	if (scope.listVisible) {
            		...
            		// $(element).find('.ui-select-toggle').scope().$on('uis:close', function() {
		            // 	scope.listVisible = false;
		            // });
            	}
            });        
